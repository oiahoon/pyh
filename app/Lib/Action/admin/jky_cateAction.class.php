...
class jky_cateAction extends backendAction {
    ...
    public function index() {
        ...
        $array = array();
        foreach($result as $r) {
            $r['str_img'] = $r['img'] ? '<div class="img_border"><img src="'.attach($r['img'], MODULE_NAME).'" width="26" height="26" class="J_preview" data-bimg="'.attach($r['img'], MODULE_NAME).'"/></div>' : '';
    ...
    public function ajax_upload_img() {
        ...
        if (!empty($_FILES['img']['name'])) {
            $result = $this->_upload($_FILES['img'], MODULE_NAME, array('width'=>'80', 'height'=>'80'));
            if ($result['error']) {
