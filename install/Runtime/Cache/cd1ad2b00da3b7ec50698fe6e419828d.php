...
<form action="<?php echo U('check');?>" method="post">
    <div class="c_main_title"><?php echo L('step_check_desc');?></div>
    <div class="c_main_body">
        <?php if(is_array($error)): $i = 0; $__LIST__ = $error;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div class="error_msg"><?php echo ($val); ?></div><?php endforeach; endif; else: echo "" ;endif; ?>
    </div>
    <div class="act">
        <div class="btn">
            <?php if(isset($error)): ?><input type="submit" class="btn_next" value="<?php echo L('recheck');?>"/>
            <?php else: ?>
            <input type="submit" value="<?php echo L('next');?>" class="btn_next" /><?php endif; ?>
        </div>
    </div>
</form>
</div>
